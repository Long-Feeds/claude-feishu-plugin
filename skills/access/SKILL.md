...
2. Show: dmPolicy, allowFrom count and list, pending count with codes +
   sender open_ids + age, groups count.
...
- `ackReaction`: string (Feishu emoji_type name like THUMBSUP) or `""` to disable
...
- Sender IDs are opaque strings (Feishu open_id, e.g., `ou_xxx`). Don't
  validate format.
